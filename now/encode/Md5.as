package now.encode {
	...
	public final class Md5 {
		...
		private static function F(x:uint, y:uint, z:uint):uint {
			return (x & y) | ((~x) & z);
		}

		private static function G(x:uint, y:uint, z:uint):uint {
			return (x & z) | (y & (~z));
		}

		private static function H(x:uint, y:uint, z:uint):uint {
			return (x ^ y ^ z);
		}

		private static function I(x:uint, y:uint, z:uint):uint {
			return (y ^ (x | (~z)));
		}
